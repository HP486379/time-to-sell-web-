...
- 実データ取得元
  ...
  - 重要イベント: ローカル算出（FOMC=第3水曜、CPI=月10日目安、雇用統計=月初の金曜を JST 日付のまま採用）。`backend/services/event_service.py` のヒューリスティックカレンダーをそのまま UI/ログに `source=local heuristic calendar` として出力します。
