export const tooltips = {
  score: {
    total: 'テクニカル・マクロ・イベント補正を統合した 0〜100 の売り時スコアです。スコアが高いほど「利確寄り（割高）」と判定します。',
    label:
      '総合スコアの値から自動判定したテキストです。例：80以上＝利確を強く推奨、60〜80＝利確を検討、40〜60＝ホールド、40未満＝買い増し検討。',
    technical:
      'S&P500 の価格と移動平均線（20日・60日・200日）の位置関係や乖離率から計算したスコアです。高いほどテクニカル的に割高（利確寄り）です。',
    ...
    d: 'S&P500 の現在値が 200日移動平均線からどれくらい離れているかを示す乖離率（%）です。プラスが大きいほど「長期平均より上振れ＝割高寄り」です。',
    T_base:
  ...
  position: {
    card: 'S&P500 連動の円建て投資信託について、自分の保有数量と平均取得単価（円）を入力するエリアです。',
    ...
    marketValue: '現在の S&P500 基準価額 × 保有数量 から計算した評価額（円）です。',
    pnl: '評価額 − 取得総額（平均取得単価 × 保有数量）で計算した含み損益（円）です。プラスなら含み益、マイナスなら含み損です。',
    navSynthetic: 'S&P500 価格と USD/JPY 為替を掛け合わせて算出した参考基準価額（円）です。公式値が取得できない場合の目安として利用します。',
    navOfficial: 'eMAXIS Slim 米国株式（S&P500）などの実際の基準価額（円）です。取得に失敗した場合は参考値のみを表示します。',
  },
  chart: {
    title: '直近期間の S&P500 終値と、20日・60日・200日移動平均線の推移をまとめたチャートです。横軸は日付、縦軸は指数レベルです。',
    close: 'S&P500 指数の終値の推移を表すラインです。',
    ma20: 'S&P500 の 20日移動平均線です。短期のトレンドを示します。',
    ma60: 'S&P500 の 60日移動平均線です。中期的なトレンドを示します。',
    ma200: 'S&P500 の 200日移動平均線です。長期のトレンドや「平常値」の目安として使います。',
  },
  macro: {
